...
public class BTreeFile implements DbFile {
		...
		switch (pid.pgcateg()) {
			...
			case BTreePageId.INTERNAL:
				...
				if (entry != null) {
					...
					throw new DbException("BTree internal page iterator fails");
				}
			default:
				throw new DbException("Unexpected BTreePageId: " + pid.pgcateg());
		}
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		int count = 0, numTuples = page.getNumTuples();
		while (it.hasNext()) {
			Tuple tuple = it.next();
			if (count < (numTuples + 1) / 2) {
		...
		if (newPage.getLeftSiblingId() != null) {
			BTreeLeafPage leftSibling = (BTreeLeafPage) getPage(tid, dirtypages, newPage.getLeftSiblingId(),
					Permissions.READ_WRITE);
			leftSibling.setRightSiblingId(newPage.getId());
		}

		Field middleKey = newPage.reverseIterator().next().getField(keyField);
		BTreeEntry entry = new BTreeEntry(middleKey, newPage.getId(), page.getId());
		Field pageKey = page.iterator().next().getField(keyField);
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), pageKey);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int count = 0, numEntries = page.getNumEntries();
		while (it.hasNext()) {
		...
		BTreeEntry middleKeyEntry = newPage.reverseIterator().next();
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int numSiblingTuples = sibling.getNumTuples();
		int numTuplesToMove = (numSiblingTuples - numTuples) / 2;
		if (isRightSibling) {
			Iterator<Tuple> it = sibling.iterator();
			for (int i = 0; i < numTuplesToMove; i++) {
				Tuple tuple = it.next();
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		page.insertEntry(new BTreeEntry(parentEntry.getKey(), leftSibling.reverseIterator().next().getRightChild(),
				page.iterator().next().getLeftChild()));
		Iterator<BTreeEntry> it = leftSibling.reverseIterator();
		BTreeEntry entry = null;
		while (it.hasNext()) {
			entry = it.next();
			if (page.getNumEntries() < leftSibling.getNumEntries() - 1) {
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		page.insertEntry(new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(),
				rightSibling.iterator().next().getLeftChild()));
		Iterator<BTreeEntry> it = rightSibling.iterator();
		BTreeEntry entry = null;
		while (it.hasNext()) {
			entry = it.next();
			if (page.getNumEntries() < rightSibling.getNumEntries() - 1) {
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage rightSibling = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(),
					Permissions.READ_WRITE);
